...
function buildTerminalActionsCard() {
...
export function handleAdaptiveCardTestCommand(rawText) {
    const trimmed = rawText.trim();
    if (!trimmed.startsWith("/test-card"))
        return null;
    const [, ...rest] = trimmed.split(/\s+/);
    const variant = (rest[0] || "basic").trim().toLowerCase();
    if (variant === "list" || variant === "help")
        return buildHelpResult();
    if (variant === "basic")
        return buildBasicCard();
    if (variant === "choices")
        return buildChoicesCard();
    if (variant === "approval")
        return buildApprovalCard();
    if (variant === "completed")
        return buildCompletedCard();
    if (variant === "failed")
        return buildFailedCard();
    if (variant === "cancelled")
        return buildCancelledCard();
    if (variant === "bad-url")
        return buildBadUrlCard();
    if (variant === "submit-error")
        return buildSubmitErrorCard();
    if (variant === "stay-open")
        return buildStayOpenCard();
    if (variant === "terminal-actions")
        return buildTerminalActionsCard();
    return {
        status: "error",
